...
void Game::postUpdate() {
  ...
  if (game_mode == DROP_MODE && (!physics->checkActive(character->identity) || physics->getVelocity(character->identity) < 0.00001)) {
    game_mode = PREP_MODE;
...
void Game::update() {
  ...
  if (game_mode == POWER_MODE) {
    if (shot_rising) character->shot_power += 0.25f;
    else character->shot_power -= 0.25f;
  ...
  //printf("Velocity = %f\n", physics->getVelocity(character->identity));
  //printf("Upshot %d\n", character->up_shot);
...
void Game::render() {
  ...
  if (game_mode == POWER_MODE) {
    ...
    glBegin(GL_QUADS);
    glTexCoord2d(0.0,1.0 - 140.0 / 150.0 * character->shot_power / character->default_shot_power); glVertex2d(power_x + 0.0,power_y + 150 - 140 * character->shot_power / character->default_shot_power);
    glTexCoord2d(0.0,1.0); glVertex2d(power_x + 0.0,power_y + 150);
    glTexCoord2d(1.0,1.0); glVertex2d(power_x + 30,power_y + 150);
    glTexCoord2d(1.0,1.0 - 140.0 / 150.0 * character->shot_power / character->default_shot_power); glVertex2d(power_x + 30,power_y + 150 - 140 * character->shot_power / character->default_shot_power);
    glEnd();
  }
...
// void Game::renderDesign() {
//   glLoadIdentity();
//   gluLookAt(character->position->x - 5, character->position->y - 5, 5,
//             character->position->x, character->position->y, 0,
//             0, 0, 1);
  ...
  if(game_mode == PREP_MODE) {
    ...
    if(e.type == SDL_MOUSEBUTTONUP) {
      ...
      if (character->up_shot) {
        // a 45 degree shot
        character->impulse(0.293 * character->shot_power * sin(character->shot_rotation), 0.293 * -character->shot_power * cos(character->shot_rotation), 0.707 * character->shot_power);
      } else {
        character->impulse(character->shot_power * sin(character->shot_rotation), -character->shot_power * cos(character->shot_rotation), 0.5);
      }
      physics->activate(character->identity);
    }
  }
...
bool Game::initializeLighting() {
  ...
  // GLfloat globalAmbient[] = {0.7f,1.0f,1.0f,1.0f};
  ...
  //   //
  //   0,0,1//spot Cutoff,spot Exponent,enabled
  // );
